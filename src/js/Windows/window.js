...
/**
 * @typedef {'center' | 'cascade' | 'random' | 'fixed'} WindowPositionStrategy
 */
 ...
 * @property {number} [x] - Default window x position (used with 'fixed' strategy or as hint)
 * @property {number} [y] - Default window y position (used with 'fixed' strategy or as hint)
 * @property {WindowPositionStrategy} [position] - Positioning strategy: 'center' (default), 'cascade', 'random', or 'fixed'
...
/** @type {number} - Static counter for cascade positioning */
let cascadeOffset = 0;
...
export default class Window extends EventEmitter {
  ...
  constructor(Id, config) {
    ...
    // Bind methods once to enable proper event listener cleanup
    this.handleResize = this.handleResize.bind(this);
    this.resize = this.resize.bind(this);
    this.endResize = this.endResize.bind(this);

    // Calculate smart position that ensures window is visible
  ...
  /**
   * Calculates a valid position for the window ensuring it's visible within the viewport.
   * Supports multiple positioning strategies.
   * @private
   * @param {number|undefined} requestedX - The requested X position
   * @param {number|undefined} requestedY - The requested Y position
   * @param {WindowPositionStrategy} [strategy='center'] - The positioning strategy
   * @returns {{x: number, y: number}} - The calculated position
   */
  #calculatePosition(requestedX, requestedY, strategy = "center") {
    const viewportWidth = window.innerWidth;
    const viewportHeight = window.innerHeight;
    const taskbarHeight = 35; // Account for taskbar at top
    const padding = 20; // Minimum padding from edges

    // Calculate maximum allowed positions
    const maxX = Math.max(padding, viewportWidth - this.width - padding);
    const maxY = Math.max(
      taskbarHeight + padding,
      viewportHeight - this.height - padding,
    );
    ...
    switch (strategy) {
      case "fixed":
        // Use exact requested position, clamped to visible area
        x = Math.max(padding, Math.min(requestedX ?? padding, maxX));
        y = Math.max(
          taskbarHeight + padding,
          Math.min(requestedY ?? taskbarHeight + padding, maxY),
        );
        break;

      case "cascade":
        // Cascade windows diagonally from top-left
        const cascadeStep = 30;
        const baseX = 50;
        const baseY = taskbarHeight + 20;

        x = baseX + cascadeOffset * cascadeStep;
        y = baseY + cascadeOffset * cascadeStep;

        // Reset cascade if window would go off screen
        if (x > maxX || y > maxY) {
          cascadeOffset = 0;
          x = baseX;
          y = baseY;
        ...
        y = Math.max(taskbarHeight + padding, Math.min(y, maxY));
      ...
      case "random":
        ...
        y =
          taskbarHeight +
          padding +
          Math.random() * (maxY - taskbarHeight - padding);
        break;

      case "center":
      default:
        // If x/y provided, use as hint but ensure visibility
        if (requestedX !== undefined && requestedY !== undefined) {
          x = Math.max(padding, Math.min(requestedX, maxX));
          y = Math.max(taskbarHeight + padding, Math.min(requestedY, maxY));
        } else {
          // Center the window
          x = Math.max(padding, (viewportWidth - this.width) / 2);
          y = Math.max(
            taskbarHeight + padding,
  ...
  /**
   * Resets the cascade offset counter. Call this when clearing all windows.
   * @static
   */
  static resetCascade() {
    cascadeOffset = 0;
  ...
  async #createElement() {
    ...
    if (this.#config?.initialURL)
      await this.fetchWindowContents(this.#config.initialURL);
  }
  ...
  createResizeHandles() {
    ...
    resizeHandles.forEach((handle) => {
      ...
      resizeHandle.addEventListener("mousedown", (e) =>
        this.startResize(e, handle.dx, handle.dy),
      );
  ...
  /**
   * Removes the window from the DOM and cleans up event listeners
   */
  destroy() {
  ...
  async handleGameContent(page) {
    ...
    const styles = Array.from(
      page.querySelectorAll('style, link[rel="stylesheet"]'),
    );
    for (const style of styles) {
  ...
  changeTaskbarTitle(title) {
    this.emit("changeTaskbarTitle", { id: this.id, title: title });
  }
}
